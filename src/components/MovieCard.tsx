...
const MovieCard = ({ id, poster_path, title, vote_average, release_date }: Partial<Movie>) => {
  return (
      ...
      <Link to={`/movie/${id}`}>
        <div className="w-full max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl transform transition-transform duration-200 hover:scale-[1.03] mx-auto">
          <img
            src={
              poster_path
            ...
            alt={title}
            className="rounded-lg w-full h-[250px] sm:h-[450px] object-cover shadow-lg"
          />
